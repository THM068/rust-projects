...
package connectors:
  ...
  extension (seq: Seq[(String, String)])
    ...
    def withXCorrelationId(value: Option[String] = None)(implicit headerCarrier: HeaderCarrier): Seq[(String, String)] = {
      val xCorrelationId = stripSession(value.getOrElse(headerCarrier.requestId.map(_.value).getOrElse(UUID.randomUUID().toString)))
    ...
    def withDate(value: Option[String] = None): Seq[(String, String)] = {
      ...
      val date      = ZonedDateTime.now(ZoneId.of("Europe/London")).format(formatter)

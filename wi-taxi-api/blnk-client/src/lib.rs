...
impl BlnkClient {
    ...
    /// Create a new ledger
    pub async fn create_ledger(&self, request: CreateLedgerRequest) -> Result<ApiResponse<Ledger>, ReqwestError> {
    ...
    /// Get a ledger by ID
    pub async fn get_ledger(&self, ledger_id: &str) -> Result<ApiResponse<Ledger>, ReqwestError> {
    ...
    /// List all ledgers
    pub async fn list_ledgers(&self) -> Result<ApiResponse<Vec<Ledger>>, ReqwestError> {
        let url = format!("{}/ledgers", self.base_url);
        let response = self.build_request(&url).send().await?;
        response.json().await
    }
    ...
    /// Get balance by ID
    pub async fn get_balance(&self, balance_id: &str) -> Result<ApiResponse<Balance>, ReqwestError> {
    ...
    match client.create_ledger(ledger_request).await {
        Ok(ledger) => {
            ...
            let account_request = CreateAccountRequest {
                ...
                ledger_id: ledger.data.ledger_id.clone(),

//! The wi_taxi_api-macros crate contains the `test` macro.
...
/// ```
///
/// Test functions marked with this attribute receive a [`wi_taxi_api-web::test_helpers::TestContext`] struct via which they get access a preconfigured instance of the application. The application instance is extended with convenience methods for making requests from the test.
...
pub fn test(_: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let setup = quote! {
        let context = wi_taxi_api_web::test_helpers::setup().await;
    };

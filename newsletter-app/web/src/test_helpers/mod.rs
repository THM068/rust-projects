...
use newsletter_app_config::{load_config, Config, Environment};
...
///
/// A `TestContext` is passed as an argument to tests marked with the [`newsletter_app_macros::test`] attribute macro. It is used to access the application under test.
///
/// Example:
/// ```
/// #[test]
...
///
/// This function initializes a new instance of the application under test using the configuration for [`newsletter_app_config::Environment::Test`].
///
/// This function is not invoked directly but used inside of the [`newsletter_app_macros::test`] attribute macro. The test context is automatically passed to test cases marked with that macro as an argument.
pub async fn setup() -> TestContext {
    ...
    let app = init_routes(AppState {blnk_client: BlnkClient::new("http://localhost:5001/")});

use spring_rs_gerust_config::Config;
use spring_rs_gerust_db::{connect_pool, DbPool};
use std::sync::Arc;

/// The application's state that is available in [`crate::controllers`] and [`crate::middlewares`].
pub struct AppState {
    /// The database pool that's used to get a connection to the application's database (see [`spring_rs_gerust_db::DbPool`]).
    pub db_pool: DbPool,
}
...
///
/// This function creates an [`AppState`] based on the current [`spring_rs_gerust_config::Config`].

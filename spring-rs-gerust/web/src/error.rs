...
pub enum Error {
    ...
    Database(#[from] spring_rs_gerust_db::Error),
...
impl IntoResponse for Error {
    fn into_response(self) -> axum::response::Response {
        match self {
            Error::Database(spring_rs_gerust_db::Error::NoRecordFound) => {
                StatusCode::NOT_FOUND.into_response()
            }
            Error::Database(spring_rs_gerust_db::Error::ValidationError(e)) => {
                validation_error(e).into_response()
            }
            Error::Database(spring_rs_gerust_db::Error::DbError(e)) => {
                internal_error(e).into_response()
            }
            Error::Database(spring_rs_gerust_db::Error::RecordAlreadyExists) => (

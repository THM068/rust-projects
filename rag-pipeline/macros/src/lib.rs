//! The rag_pipeline-macros crate contains the `test` macro.
...
/// ```
///
/// Test functions marked with this attribute receive a [`rag_pipeline-web::test_helpers::TestContext`] struct via which they get access a preconfigured instance of the application. The application instance is extended with convenience methods for making requests from the test.
...
pub fn test(_: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let setup = quote! {
        let context = rag_pipeline_web::test_helpers::setup().await;
    };
